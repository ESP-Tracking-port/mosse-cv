		...
		if (aInitial) {  // B = eta * complexMult(gaussfft, conj(imagefft))
			...
			for (unsigned row = 0; row < roi().rows(); ++row) {
				for (unsigned col = 0; col < roi().cols(); ++col) {
					...
					Ut::mulA3<Tp::Repr::StorageF32 | Tp::Repr::ReprRaw, ReprAb, ReprAb>(eta(), aPrev, aPrev);
					Ut::mulA3<Tp::Repr::StorageF32 | Tp::Repr::ReprRaw, ReprAb, ReprAb>(eta(), aPrevImag, aPrevImag);
